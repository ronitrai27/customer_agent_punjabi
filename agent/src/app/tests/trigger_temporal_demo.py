...
class Handler(http.server.SimpleHTTPRequestHandler):
    def translate_path(self, path):
        ...
        print(
            f"[HTTP Server Debug] Requested: {path} | DIRECTORY: {DIRECTORY} | Resolved: {resolved_path} | Exists: {os.path.exists(resolved_path)}"
        )
...
def start_http_server():
    ...
    with socketserver.TCPServer(("", PORT), Handler) as httpd:
        print(
            f"[HTTP Server] Hosting files from '{DIRECTORY}' at http://127.0.0.1:{PORT}"
        )
        httpd.serve_forever()
...
async def trigger_workflow():
    ...
    try:
        ...
        print(
            "[!] Temporal Server is offline. Falling back to direct local pipeline execution for demo..."
        )
    ...
    if client:
        print("  Execution Mode: Temporal Workflow")
        print(f"  Workflow ID:    {workflow_id}")
    else:
        print("  Execution Mode: Direct Local Execution (Temporal Offline)")
