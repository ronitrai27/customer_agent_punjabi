...
class StatusManager:
    ...
    def __init__(self):
        ...
        if self.redis_url:
            try:
                ...
                self.client.ping()
                logger.info(
                    "Successfully connected to Upstash Redis for status tracking."
                )
            except Exception as e:
                logger.error(f"Failed to connect to Upstash Redis: {e}")
        else:
            logger.warning(
                "UPSTASH_REDIS_URL is not configured. Job status tracking will be inactive."
            )
    ...
    def update_status(
        self, job_id: str, step: int, message: str, status: str = "processing"
    ) -> None:
        """
        ...
        try:
            ...
            self.client.publish(f"job-channel-{job_id}", json.dumps(payload))
            logger.info(
                f"[StatusManager] Job {job_id} -> Step {step}/6: {message} ({status})"
            )
        except Exception as e:
