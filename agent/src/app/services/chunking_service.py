...
class ChunkingService:
    ...
    def _get_embedding_model(self):
        ...
        if self._embedding_model is None:
            logger.info("Initializing SentenceTransformer (all-MiniLM-L6-v2) for Semantic Chunking...")
            try:
                from sentence_transformers import SentenceTransformer
                self._embedding_model = SentenceTransformer("all-MiniLM-L6-v2")
                logger.info("SentenceTransformer model loaded successfully.")
            except ImportError:
                logger.error("sentence-transformers is not installed. Semantic chunking will fall back to recursive chunking.")
                raise ImportError(
                    "Please install 'sentence-transformers' to use semantic chunking."
                )
        return self._embedding_model

    def split_text_recursive(self, text: str, max_chars: int = 1500, overlap_chars: int = 200) -> List[str]:
        """
        ...
        for split in splits:
            ...
            if current_length + item_len > max_chars:
                ...
                if item_len > max_chars:
                    sub_chunks = self.split_text_recursive(item, max_chars, overlap_chars)
    ...
    def chunk_by_structure(
        ...
        def process_current_section():
            ...
            sub_splits = self.split_text_recursive(section_content, max_chars, overlap_chars)
        ...
        logger.info(f"[Step 3 - Structure-Aware] Generated {len(chunks)} structure-aware chunks.")
        return chunks
    ...
    def chunk_semantically(
        ...
        try:
            ...
            logger.warning(f"Could not load embedding model: {e}. Falling back to structure-aware/recursive.")
        ...
        for idx, sentence in enumerate(sentences):
            ...
            if should_split:
                ...
                logger.debug(f"Creating semantic chunk {chunk_index} (sim drop to {sim_value:.3f})")
