...
class EmbeddingService:
    ...
    def _get_local_model(self):
        ...
        if self._local_model is None:
            logger.info(f"Initializing local SentenceTransformer ({self.model_name}) for dense embeddings...")
            try:
                        ...
                        if response.status_code == 200:
                            ...
                            if isinstance(embeddings, list):
                                if len(embeddings) > 0 and isinstance(embeddings[0], dict):
                                    ...
                                    embeddings = [emb.get("embedding", emb) for emb in embeddings]
                                all_embeddings.extend(embeddings)
                            else:
                                raise ValueError(f"Unexpected response format from TEI: {embeddings}")
                        else:
                            # Try OpenAI style wrapper (vLLM / Ollama compatible)
                            logger.warning(f"TEI style request failed with status {response.status_code}. Retrying OpenAI-embeddings format...")
                            ...
                            if response_oa.status_code == 200:
                                ...
                                raise RuntimeError(f"Embedding API request failed: {response_oa.text}")
                                
                return all_embeddings
            except Exception as e:
                logger.error(f"Remote embedding extraction failed: {e}. Falling back to local model.")
        
        # Local SentenceTransformer Fallback
        logger.info("[Step 4 - Embedding] Generating dense embeddings locally using SentenceTransformer...")
    ...
    def get_sparse_embeddings(self, texts: List[str]) -> List[Dict[str, Any]]:
        ...
        """
        logger.info(f"[Step 4 - Embedding] Generating sparse TF vectors locally for {len(texts)} chunks...")
        sparse_vectors = []
