...
class IngestPipeline:
    ...
    async def run(
        ...
        status_manager.update_status(doc_id, 1, "Downloading document from storage S3 bucket...")
        ...
        try:
            ...
            status_manager.update_status(doc_id, 0, f"Download/Parsing failed: {str(e)}", status="failed")
        ...
        status_manager.update_status(doc_id, 3, "Splitting document into combined semantic-hierarchical chunks...")
        ...
        try:
            ...
            print(f"[-] PIPELINE FAILED: {e}")
            status_manager.update_status(doc_id, 0, f"Chunking failed: {str(e)}", status="failed")
        ...
        if not chunks_output:
            ...
            status_manager.update_status(doc_id, 0, "No text chunks generated from document.", status="failed")
        ...
        status_manager.update_status(doc_id, 4, "Generating dense (1024d) and sparse TF embeddings...")
        ...
        try:
            ...
            print(f"[-] PIPELINE FAILED: {e}")
            status_manager.update_status(doc_id, 0, f"Embedding generation failed: {str(e)}", status="failed")
        ...
        status_manager.update_status(doc_id, 5, "Checking Pinecone index and clearing older revisions...")
        ...
        try:
            ...
            status_manager.update_status(doc_id, 6, f"Batch upserting {len(chunks_output)} chunks to Pinecone...")
            ...
            print(f"[-] PIPELINE FAILED: {e}")
            status_manager.update_status(doc_id, 0, f"Pinecone storage failed: {str(e)}", status="failed")
