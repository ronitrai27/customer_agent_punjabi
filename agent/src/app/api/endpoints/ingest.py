...
from temporalio.client import RPCError, WorkflowExecutionStatus
...
class IngestRequest(BaseModel):
    file_url: str = Field(
        ..., description="The direct download URL of the uploaded file on Wasabi"
    )
    ...
    permissions: list[str] = Field(
        ["read:all"], description="User permissions/roles for access control"
    )
    version: str = Field("1.0.0", description="Document revision version")
    job_id: str | None = Field(
        None, description="Optional pre-generated job/document ID"
    )
...
async def start_ingestion(request: IngestRequest):
    ...
    try:
        ...
        status_manager.update_status(
            job_id, 0, f"Failed to start workflow: {str(e)}", status="failed"
        )
        raise HTTPException(
            status_code=500, detail=f"Failed to start Temporal workflow: {str(e)}"
        )

    return {
        "success": True,
        "message": "Ingestion workflow started successfully via Temporal",
        "data": {"job_id": job_id, "workflow_id": workflow_id, "status": "processing"},
    }
...
async def get_job_status(job_id: str):
    ...
    try:
        ...
        if desc.status == WorkflowExecutionStatus.COMPLETED:
            ...
            response_data["error"] = (
                "Workflow execution failed. Check worker logs or Temporal Web UI for details."
            )
        ...
        if "not found" in str(e).lower() or e.status.name == "NOT_FOUND":
            raise HTTPException(
                status_code=404, detail="Job not found in Temporal history"
            )
...
async def delete_document(doc_id: str, tenant: str = "default"):
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Document {doc_id} successfully deleted from Pinecone.",
        }
    except Exception as e:
